#include <stdio.h>
#include <stdbool.h>
#include "leer_linea.h"

#define EXITO 0
#define ERROR -1
#define EXIT -2

int main(int argc, char *argv[])
{
	printf("Ingresa tu nombre: ");

	char *nombre = leer_linea();

	if (nombre == NULL) {
		printf("Ha ocurrido un error a la hora de solicitar memoria\n\n"
		       "Cerrando programa...");
		return ERROR;
	}
	printf("\nBienvenido profesor/a %s\nIngresa los nombres de los alumnos al sistema\n\n",
	       nombre);

	printf("Recueda que para salir del programa debes de ingresar 'Salir'\n\n");

	size_t tamanio_l = 10;
	char *linea = malloc(tamanio_l * sizeof(char));

	if (linea == NULL) {
		printf("Ha ocurrido un error a la hora de solicitar memoria\n\n"
		       "Cerrando programa...");
		return ERROR;
	}

	printf("Ingresa un nombre: ");

	bool salir = false;

	bool error = false;

	int c;

	while (!salir && !error &&
	       (c = leer_linea_ptr(&linea, &tamanio_l)) != ERROR) {
		if (c == EXIT) {
			salir = true;
			printf("\nSaliendo del programa...\n");
		} else if (c == ERROR) {
			printf("Ha ocurrido un error a la hora de leer el texto introducido.\n"
			       "Verifica que sea válido lo mandado por cosola\n"
			       "Cerrado el programa...");
			error = true;
		} else {
			printf("\nHas agregado un nuevo alumno, nombre: %s\n",
			       linea);

			printf("El largo del nombre %d\n\n", c);

			printf("Ingresa otro nombre: ");
		}
	}

	free(linea);

	free(nombre);

	return EXITO;
}